...
| `BUILT_IN_FORGE_API_URL` | Yes for current LLM/storage adapters | Server-side Manus Forge service origin. | Replace the LLM and object-storage adapters when moving outside Manus. |
| `BUILT_IN_FORGE_API_KEY` | Yes for current LLM/storage adapters | Server-side Manus Forge credential. | Store only in the server-side secret manager; never expose it to the frontend. |
