...
| `RESEND_API_KEY` | Yes, for password reset | Server-side Resend API credential used by `server/_core/email.ts` to send password-reset emails. | Store only in the server-side secret manager; never expose it to the frontend. Without this set, `auth.requestPasswordReset` still runs (and still returns its generic success response, to avoid leaking whether an email is registered) but the send itself fails and is logged server-side. |
| `RESEND_FROM_EMAIL` | Optional | The `From` address for password-reset emails. | Defaults to Resend's own sandbox sender (`onboarding@resend.dev`), which works without domain verification but is only suitable for testing. Set this to an address on a domain you've verified with Resend before real users depend on password reset. |
