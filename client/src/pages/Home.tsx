...
type DemoScenario = "specialist-advisor" | "civil-service";

// Mirrors server/demoData.ts's demoScenarios -- kept as display-only copy
// here since the client can't import the server module, and this is only
// ever used to label the toggle/badges below before the matching workspace
// has loaded.
const DEMO_SCENARIOS: { id: DemoScenario; personName: string; roleLine: string; targetLine: string }[] = [
  { id: "specialist-advisor", personName: "Sarah Mwangi", roleLine: "a made-up Grade 6 specialist advisor going for Grade 7", targetLine: "checked against a traditional person specification" },
  { id: "civil-service", personName: "Amara Okafor", roleLine: "a made-up Civil Service EO going for HEO", targetLine: "checked against Success Profile behaviour statements" },
];
...
// A soft, distinct tint per objective card icon -- kept deliberately
// separate from the shared --accent token, which the redesign repointed
// to a subtle neutral hover color (shadcn's convention for it) rather
// than a saturated brand tint, so leaning on it here would have made
// Objective B blend into A and C instead of standing apart. Orange is
// used nowhere else in the app's status-color vocabulary (emerald/sky/
// violet/rose/amber/stone), so it can't be mistaken for one of those
// meanings the way another amber-adjacent tone might.
const objectiveIconTint: Record<Objective, { bg: string; fg: string }> = {
  A: { bg: "bg-secondary", fg: "text-secondary-foreground" },
  B: { bg: "bg-orange-100", fg: "text-orange-700" },
...
export default function Home() {
  ...
  const [demoScenario, setDemoScenario] = useState<DemoScenario>("specialist-advisor");
  const demoQuery = trpc.demo.workspace.useQuery({ scenario: demoScenario });
  ...
  // Once results exist for the current objective, the three-card picker
  // sitting above them made it genuinely ambiguous whether you needed to
  // pick again before the results below counted -- confirmed during a
  // fresh-eyes walkthrough. Collapsing to a one-line summary once there's
  // something to summarise (with an explicit "Change" to get the cards
  // back) removes that ambiguity without hiding the ability to switch.
  ...
  const hasResults = assessmentByRequirement.size > 0;
  ...
  function openUploadFor(documentType: "evidence" | "target" | "current_role") {
    ...
    setUploadDraft({ title: "", documentType, sourceKind: documentType === "target" ? "Job description / criteria" : documentType === "current_role" ? "Current role responsibilities" : "CV", sourceText: "" });
    setUploadOpen(true);
  }
  ...
  return (
    <div className="flex min-h-screen flex-col bg-background text-foreground">
      ...
      <main id="top" className="mx-auto w-full max-w-[1540px] flex-1 px-5 pb-10 pt-6 lg:px-8">
        {/* Print-only masthead: the on-screen header (logo, nav, sign-in) is
            hidden for print (see its print:hidden class), so a printed
            report needs its own minimal identification instead of arriving
            headerless. */}
        <div className="mb-4 hidden print:block">
          <p className="font-serif text-2xl font-semibold">ProveMyCV — CV Optimisation Report</p>
          <p className="text-sm text-muted-foreground">Generated {new Date().toLocaleDateString(undefined, { day: "numeric", month: "long", year: "numeric" })}{active?.profile?.currentRole ? ` · ${active.profile.currentRole}` : ""}{objective ? ` · ${objectives[objective].title}` : ""}</p>
        </div>
        ...
        <div className="mt-6 print:block">
          ...
          <div className="min-w-0">
            {/* A genuinely fresh signed-in account (zero documents) used to
                drop straight into a page showing four or five equally-weighted
                empty sections at once -- objective picker, document library,
                job description, current role -- with nothing marking which to
                do first. This banner gives that one moment a single, obvious
                next step; it disappears for good the instant a first document
                exists, so it never competes with the sections below once
                there's something to actually pick between. */}
            {isAuthenticated && !isGuest && !isDemo && (workspaceQuery.data?.documents?.length ?? 0) === 0 && <div className="rise mb-6 rounded-xl border border-primary/25 bg-primary/[.045] p-5 lg:p-6 print:hidden">
              <p className="font-mono text-[11px] font-bold uppercase tracking-[0.16em] text-primary">Start here</p>
              <h2 className="mt-1 font-serif text-2xl font-semibold">Add your first document</h2>
              <p className="mt-1 max-w-2xl text-sm text-muted-foreground">Paste in your CV, an appraisal, or anything similar — that's what we'll check against a job description. Everything else on this page can wait until you've added something.</p>
              <Button className="mt-4" onClick={() => openUploadFor("evidence")}><Upload />Add your evidence</Button>
            </div>}
            <section className="rise rounded-xl border bg-card p-5 shadow-sm lg:p-6 print:hidden"><div className="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between"><div><p className="font-mono text-[11px] font-bold uppercase tracking-[0.16em] text-primary">{hasResults && !pickerExpanded ? "Checked against" : "Pick one"}</p><h2 className="mt-1 font-serif text-3xl font-semibold">{hasResults && !pickerExpanded ? objectives[objective].title : "What do you want help with?"}</h2><p className="mt-1 text-xs text-muted-foreground">{hasResults && !pickerExpanded ? "The results below are for this — pick \"Change\" if you meant a different one." : isAuthenticated ? "Pick an objective, then review your evidence whenever you're ready." : "Try it free, right now — no account needed. Sign in only if you want to save your results for later."}</p></div><div className="flex shrink-0 gap-2">{hasResults && !pickerExpanded && <Button variant="outline" onClick={() => setPickerExpanded(true)}>Change</Button>}<Button onClick={() => { setAnalysisError(null); setPickerExpanded(false); if (isAuthenticated) runAnalysis.mutate({ objective }); else setGuestOpen(true); }} disabled={isAuthenticated && isAnalysing}>{(isAuthenticated && isAnalysing) ? <Loader2 className="animate-spin" /> : <Sparkles />}{isAuthenticated ? (hasResults ? "Check again" : "Review my evidence") : <>Review my evidence<span className="hidden sm:inline"> — no sign-in needed</span></>}</Button></div></div>{(!hasResults || pickerExpanded) && <div className="mt-5 grid gap-3 lg:grid-cols-3">{(Object.keys(objectives) as Objective[]).map(key => { const item = objectives[key]; const Icon = item.icon; const tint = objectiveIconTint[key]; return <button key={key} onClick={() => chooseObjective(key)} className={`rounded-lg border p-4 text-left shadow-sm ring-1 transition-all duration-200 hover:-translate-y-0.5 hover:shadow-md ${objective === key ? "border-primary bg-primary/[.035] ring-primary/20" : "bg-card ring-primary/0 hover:border-primary/40"}`}><div className="flex items-center justify-between"><span className={`grid size-8 place-items-center rounded-lg ${tint.bg} ${tint.fg}`}><Icon className="size-4" /></span><span className="font-mono text-[11px] font-bold uppercase tracking-[.14em] text-muted-foreground">{item.label}</span></div><p className="mt-4 font-semibold">{item.title}</p><p className="mt-1 text-xs leading-5 text-muted-foreground">{item.description}</p></button>; })}</div>}
              {/* Same silent-wait problem as the guest dialog, same fix:
                  the button alone going into a spinner state for 60-100s,
                  with nothing else on the page changing, reads as "did that
                  work?" long before a real Gemini analysis actually finishes. */}
              {isAuthenticated && isAnalysing && <div className="mt-5 flex items-center gap-3 rounded-lg border border-primary/20 bg-primary/[.04] px-4 py-3 text-sm"><Loader2 className="size-5 shrink-0 animate-spin text-primary" /><p><strong className="font-semibold">Checking your evidence against {objectives[objective].title.toLowerCase()}.</strong> This usually takes under a minute — the page will update on its own.</p></div>}
            </section>

            {analysisError && <div className="print:hidden"><AnalysisFailureAlert message={analysisError} /></div>}
            ...
            {activeSection === "library" && <EvidenceLibrary active={active} isAuthenticated={isAuthenticated} isDemo={isDemo} profileOpen={profileOpen} setProfileOpen={setProfileOpen} profileDraft={profileDraft} setProfileDraft={setProfileDraft} onProfileSubmit={handleProfile} onOpenUpload={openUploadFor} onLoadDemo={() => { if (ensureAuth()) loadDemo.mutate({ scenario: demoScenario }); }} demoLoading={loadDemo.isPending} onDeleteDocument={(documentId: number) => { setDeletingDocumentId(documentId); deleteDocument.mutate({ documentId }); }} deletingDocumentId={deletingDocumentId} onDeleteAccount={handleDeleteAccount} deleteAccountPending={deleteAccountPending} />}
      ...
      <div className="mt-auto border-t border-primary/20 bg-primary px-4 py-3 text-center text-[11px] leading-5 text-primary-foreground"><p>This tool provides decision support from supplied evidence only. It does not determine employment eligibility, banding/grading decisions, legal rights, job-evaluation outcomes, or professional competence for any profession.</p><p className="mt-1 text-primary-foreground/70"><Link href="/guides/civil-service-success-profiles" className="underline-offset-2 hover:underline">Guides</Link><span className="mx-2">·</span><Link href="/privacy" className="underline-offset-2 hover:underline">Privacy</Link><span className="mx-2">·</span><Link href="/terms" className="underline-offset-2 hover:underline">Terms</Link><span className="mx-2">·</span><Link href="/contact" className="underline-offset-2 hover:underline">Contact</Link><span className="mx-2">·</span><a href="https://peerpush.com/p/provemycv" target="_blank" rel="noopener" className="underline-offset-2 hover:underline">PeerPush</a></p></div>
...
function FirstLoadEntry({ onStart, onExploreDemo, demoScenario }: { onStart: () => void; onExploreDemo: (scenario: DemoScenario) => void; demoScenario: DemoScenario }) {
  const [pickedScenario, setPickedScenario] = useState<DemoScenario>(demoScenario);
  const scenario = DEMO_SCENARIOS.find(item => item.id === pickedScenario) ?? DEMO_SCENARIOS[0]!;
  return <section className="grid gap-5 lg:grid-cols-[minmax(0,1.65fr)_minmax(300px,.75fr)]"><div className="relative overflow-hidden rounded-2xl bg-primary px-7 py-9 text-primary-foreground shadow-[0_18px_45px_-24px_rgba(14,32,50,.65)] lg:px-10 lg:py-12"><div className="absolute -right-20 -top-24 size-72 rounded-full border border-white/15" /><div className="absolute -bottom-24 right-24 size-52 rounded-full border border-white/10" /><div className="relative max-w-2xl"><span className="inline-flex items-center gap-2 rounded-full bg-white/15 px-3 py-1.5 font-mono text-[11px] font-bold uppercase tracking-[.18em] text-white"><span className="size-1.5 shrink-0 rounded-full bg-emerald-400" />Get started</span><h1 className="mt-4 font-serif text-4xl font-semibold leading-[1.04] tracking-tight sm:text-5xl">See what your CV actually proves.</h1><p className="mt-3 font-serif text-lg font-semibold italic text-white/90 sm:text-xl">Your next role starts with what you can already prove.</p><p className="mt-4 max-w-xl text-base leading-7 text-white/80">Paste in your CV (or anything else — an appraisal, feedback, a certificate) and the job description or behaviour criteria you're aiming for. We'll show you exactly what matches, right here, no account needed.</p><div className="mt-8 flex flex-col gap-3 sm:flex-row sm:items-center"><Button size="lg" onClick={onStart} className="bg-white px-6 text-primary shadow-sm hover:bg-white/90"><Sparkles className="size-4" />Start now — it's free</Button><span className="text-sm text-white/70">Takes about 5 minutes</span></div><div className="mt-8 grid gap-3 border-t border-white/15 pt-5 sm:grid-cols-3"><EntryStep number="01" label="Paste in your evidence" /><EntryStep number="02" label="Paste in the criteria" /><EntryStep number="03" label="See exactly what matches" /></div></div></div><aside className="rounded-2xl border bg-card p-6 shadow-sm"><Badge variant="outline" className="border-amber-300 bg-amber-50 text-amber-800">Example only</Badge><h2 className="mt-4 font-serif text-2xl font-semibold">Prefer to look first?</h2><div className="mt-4 flex gap-1.5 rounded-lg border bg-muted/40 p-1">{DEMO_SCENARIOS.map(item => <button key={item.id} type="button" onClick={() => setPickedScenario(item.id)} className={`flex-1 rounded-md px-2.5 py-2 text-xs font-semibold transition-colors ${pickedScenario === item.id ? "bg-card text-primary shadow-sm" : "text-muted-foreground hover:text-foreground"}`}>{item.personName}</button>)}</div><p className="mt-3 text-sm leading-6 text-muted-foreground">Take a look at {scenario.personName}'s example — {scenario.roleLine}, {scenario.targetLine}. It shows you what your results would look like, using made-up data.</p><Button variant="outline" className="mt-6 w-full justify-between" onClick={() => onExploreDemo(pickedScenario)}>See the example <ArrowRight className="size-4" /></Button><p className="mt-4 text-xs leading-5 text-muted-foreground">You can start your own review any time — the example is just there if you want to see it first.</p></aside></section>;
}
function EntryStep({ number, label }: { number: string; label: string }) { return <div className="flex items-center gap-2 text-xs leading-4 text-white/80"><span className="grid size-6 shrink-0 place-items-center rounded-md border border-white/30 font-mono text-[11px] font-bold text-white">{number}</span>{label}</div>; }
