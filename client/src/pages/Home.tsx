    ...
    <div className="min-h-screen bg-background text-foreground">
      ...
      <main id="top" className="mx-auto max-w-[1540px] px-5 pb-28 pt-6 lg:px-8">
        ...
        <div className="mt-6 grid gap-6 xl:grid-cols-[240px_minmax(0,1fr)]">
          <aside className="xl:sticky xl:top-24 xl:h-fit"><nav className="rounded-xl border bg-card p-2 shadow-sm"><NavButton active={activeSection === "evidence-map"} icon={SearchCheck} label="Your strengths and gaps" onClick={() => setActiveSection("evidence-map")} /><NavButton active={activeSection === "library"} icon={FolderOpen} label="My documents" onClick={() => setActiveSection("library")} /><NavButton active={activeSection === "objective-summary"} icon={BookOpenText} label="My results" onClick={() => setActiveSection("objective-summary")} /><NavButton active={activeSection === "safety"} icon={ShieldCheck} label="How this works" onClick={() => setActiveSection("safety")} /></nav><div className="mt-4 rounded-xl border border-dashed bg-card p-4"><p className="font-mono text-[10px] font-medium uppercase tracking-[0.14em] text-muted-foreground">What you're working on</p><p className="mt-1 font-serif text-lg font-semibold">{objectives[objective].title}</p><p className="mt-1 text-xs leading-5 text-muted-foreground">{objectives[objective].label}</p></div></aside>

          <div className="min-w-0">
            <section className="rise rounded-xl border bg-card p-5 shadow-sm lg:p-6"><div className="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between"><div><p className="font-mono text-[10px] font-bold uppercase tracking-[0.16em] text-primary">Pick one</p><h2 className="mt-1 font-serif text-2xl font-semibold">What do you want help with?</h2><p className="mt-1 text-xs text-muted-foreground">Try it free, right now — no account needed. Sign in only if you want to save your results for later.</p></div><Button onClick={() => { setAnalysisError(null); if (isAuthenticated) runAnalysis.mutate({ objective }); else setGuestOpen(true); }} disabled={isAuthenticated && isAnalysing} className="shrink-0">{(isAuthenticated && isAnalysing) ? <Loader2 className="animate-spin" /> : <Sparkles />}{isAuthenticated ? "Check my evidence" : "Check my evidence — no sign-in needed"}</Button></div><div className="mt-5 grid gap-3 lg:grid-cols-3">{(Object.keys(objectives) as Objective[]).map(key => { const item = objectives[key]; const Icon = item.icon; return <button key={key} onClick={() => chooseObjective(key)} className={`rounded-lg border p-4 text-left transition-all duration-150 hover:-translate-y-0.5 hover:shadow-sm ${objective === key ? "border-primary bg-primary/[.035] ring-1 ring-primary/20" : "bg-card hover:border-primary/40"}`}><div className="flex items-center justify-between"><span className="grid size-8 place-items-center rounded-md bg-secondary text-secondary-foreground"><Icon className="size-4" /></span><span className="font-mono text-[10px] font-bold uppercase tracking-[.14em] text-muted-foreground">{item.label}</span></div><p className="mt-4 font-semibold">{item.title}</p><p className="mt-1 text-xs leading-5 text-muted-foreground">{item.description}</p></button>; })}</div></section>
...
function DeleteDocumentButton({ title, pending, onConfirm }: { title: string; pending: boolean; onConfirm: () => void }) {
  return <AlertDialog>
    <AlertDialogTrigger asChild><Button variant="ghost" size="icon" className="size-7 text-muted-foreground hover:text-rose-700" aria-label={`Delete ${title}`} disabled={pending}>{pending ? <Loader2 className="size-3.5 animate-spin" /> : <Trash2 className="size-3.5" />}</Button></AlertDialogTrigger>
    <AlertDialogContent>
      <AlertDialogHeader>
...
function EvidenceLibrary({ active, isAuthenticated, isDemo, uploadOpen, setUploadOpen, profileOpen, setProfileOpen, profileDraft, setProfileDraft, onProfileSubmit, uploadDraft, setUploadDraft, uploadFile, setUploadFile, onUploadSubmit, uploadPending, onLoadDemo, demoLoading, onDeleteDocument, deletingDocumentId }: any) {
