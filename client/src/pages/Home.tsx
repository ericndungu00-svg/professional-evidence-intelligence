...
export default function Home() {
  ...
  async function handleUpload(event: FormEvent) {
    ...
    if (!uploadFile) { toast.error("Choose a TXT, PDF, or DOCX document, or paste a target specification."); return; }
...
function UploadDialog({ open, onOpenChange, draft, setDraft, file, setFile, onSubmit, pending }: any) { const isRoleText = draft.documentType === "target" || draft.documentType === "current_role"; return <Dialog open={open} onOpenChange={onOpenChange}><DialogTrigger asChild><Button><Upload />Add document</Button></DialogTrigger><DialogContent className="sm:max-w-xl"><DialogHeader><DialogTitle className="font-serif text-2xl">Add source document</DialogTitle><DialogDescription>Upload a TXT, PDF, or DOCX file up to 10 MB. You can also paste a target specification or current role description for analysis.</DialogDescription></DialogHeader><form onSubmit={onSubmit} className="grid gap-4 py-2"><Field label="Document title"><Input placeholder="e.g. Current Band 6 role description" value={draft.title} onChange={event => setDraft({ ...draft, title: event.target.value })} /></Field><div className="grid gap-4 sm:grid-cols-2"><Field label="Document purpose"><Select value={draft.documentType} onValueChange={(value: "evidence" | "target" | "current_role") => setDraft({ ...draft, documentType: value, sourceKind: value === "target" ? "Person specification" : value === "current_role" ? "Current role responsibilities" : draft.sourceKind })}><SelectTrigger><SelectValue /></SelectTrigger><SelectContent><SelectItem value="evidence">Professional evidence</SelectItem><SelectItem value="target">Target specification — Objective A</SelectItem><SelectItem value="current_role">Current role description — Objective C</SelectItem></SelectContent></Select></Field><Field label="Source category"><Input value={draft.sourceKind} onChange={event => setDraft({ ...draft, sourceKind: event.target.value })} /></Field></div><Field label={isRoleText ? "Optional source file" : "File"}><Input type="file" accept=".txt,.pdf,.docx,text/plain,application/pdf,application/vnd.openxmlformats-officedocument.wordprocessingml.document" onChange={event => setFile(event.target.files?.[0] ?? null)} />{file && <p className="mt-1 text-xs text-muted-foreground">Selected: {file.name}</p>}</Field><Field label={draft.documentType === "target" ? "Paste target specification (optional when a file is selected)" : draft.documentType === "current_role" ? "Paste current role responsibilities (optional when a file is selected)" : "Optional extracted text / transcription"}><Textarea rows={4} placeholder={draft.documentType === "target" ? "Paste the person specification or role criteria here." : draft.documentType === "current_role" ? "Paste the current job description or formal role responsibilities here." : "Use this if a scanned PDF needs a reviewed text copy. This is stored as the analysis text; the original file is retained separately."} value={draft.sourceText} onChange={event => setDraft({ ...draft, sourceText: event.target.value })} /></Field><div className="flex justify-end"><Button disabled={pending} type="submit">{pending && <Loader2 className="animate-spin" />}{isRoleText && draft.sourceText.trim() && !file ? "Store pasted role text" : "Store and extract"}</Button></div></form></DialogContent></Dialog>; }
