    ...
    <div className="flex min-h-screen flex-col bg-background text-foreground">
      <header className="sticky top-0 z-40 border-b bg-background/95 backdrop-blur">
        <div className="mx-auto flex max-w-[1540px] items-center justify-between gap-4 px-5 py-3 lg:px-8">
          ...
          <div className="flex items-center gap-2"><Button variant="ghost" size="sm" onClick={goHome} className="hidden font-semibold hover:text-primary sm:inline-flex">New analysis</Button>{isDemo && <Badge variant="outline" className="hidden border-amber-300 bg-amber-50 text-amber-800 sm:flex">Example only</Badge>}{isGuest && <Badge variant="outline" className="hidden border-primary/30 bg-primary/5 text-primary sm:flex">Not saved yet</Badge>}{isAuthenticated ? <><span className="hidden text-sm font-medium lg:inline">{user?.name}</span><Button variant="outline" size="sm" onClick={handleSignOut}>Sign out</Button></> : <Button size="sm" onClick={() => setAuthOpen(true)}><span className="sm:hidden">Sign in</span><span className="hidden sm:inline">Sign in to save your results</span></Button>}</div>
        </div>
      </header>
      ...
      <main id="top" className="mx-auto w-full max-w-[1540px] flex-1 px-5 pb-10 pt-6 lg:px-8">
        ...
        {isFirstLoad ? <FirstLoadEntry onStart={() => {
          ...
          <div className="relative max-w-4xl"><div className="mb-3 flex items-center gap-2"><span className="font-mono text-[11px] uppercase tracking-[0.14em] text-primary">About you</span>{isDemo && <Badge variant="outline" className="border-amber-300 bg-amber-50 text-amber-800">Example data</Badge>}{isGuest && <Badge variant="outline" className="border-primary/30 bg-primary/5 text-primary">Not saved</Badge>}</div><h1 className="font-serif text-4xl font-semibold tracking-tight lg:text-5xl">{active?.profile?.currentRole ?? "Tell us about yourself"}</h1><p className="mt-2 max-w-3xl text-base text-muted-foreground">{active?.profile ? `${active.profile.profession}${active.profile.specialty ? ` · ${active.profile.specialty}` : ""}${active.profile.experience ? ` · ${active.profile.experience}` : ""}` : "Try it free, right now — no account needed. Sign in only if you want to save your results for later."}</p><div className="mt-5 flex flex-wrap gap-2">{active?.profile?.targetRole && <Badge variant="secondary" className="font-medium">Aiming for: {active.profile.targetRole}</Badge>}{active?.profile?.currentLevel && <Badge variant="outline">Current level: {active.profile.currentLevel}</Badge>}{isDemo && <Badge variant="outline">Sarah Mwangi</Badge>}</div></div>
...
function NavButton({ active, icon: Icon, label, onClick }: { active: boolean; icon: typeof Target; label: string; onClick: () => void }) { return <button onClick={onClick} className={`flex w-full items-center gap-3 rounded-lg px-3 py-2.5 text-left text-sm font-medium transition-colors ${active ? "bg-primary text-primary-foreground" : "text-foreground/75 hover:bg-muted hover:text-primary"}`}><Icon className="size-4" />{label}<ChevronRight className={`ml-auto size-4 ${active ? "opacity-100" : "opacity-0"}`} /></button>; }
