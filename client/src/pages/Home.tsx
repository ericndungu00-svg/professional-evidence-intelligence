    ...
    <div className="flex min-h-screen flex-col bg-background text-foreground">
      ...
      <main id="top" className="mx-auto w-full max-w-[1540px] flex-1 px-5 pb-10 pt-6 lg:px-8">
        ...
        {isFirstLoad ? <FirstLoadEntry onStart={() => {
          ...
          <div className="relative max-w-4xl"><div className="mb-3 flex items-center gap-2"><span className="font-mono text-[11px] uppercase tracking-[0.14em] text-primary">About you</span>{isDemo && <Badge variant="outline" className="border-amber-300 bg-amber-50 text-amber-800">Example data</Badge>}{isGuest && <Badge variant="outline" className="border-primary/30 bg-primary/5 text-primary">Not saved</Badge>}</div><h1 className="font-serif text-4xl font-semibold tracking-tight text-destructive lg:text-5xl">{active?.profile?.currentRole ?? "Tell us about yourself"}</h1><p className="mt-2 max-w-3xl text-base text-muted-foreground">{active?.profile ? `${active.profile.profession}${active.profile.specialty ? ` · ${active.profile.specialty}` : ""}${active.profile.experience ? ` · ${active.profile.experience}` : ""}` : "Try it free, right now — no account needed. Sign in only if you want to save your results for later."}</p><div className="mt-5 flex flex-wrap gap-2">{active?.profile?.targetRole && <Badge variant="secondary" className="font-medium">Aiming for: {active.profile.targetRole}</Badge>}{active?.profile?.currentLevel && <Badge variant="outline">Current level: {active.profile.currentLevel}</Badge>}{isDemo && <Badge variant="outline">Sarah Mwangi</Badge>}</div></div>
        ...
        <div className="mt-6 grid gap-6 xl:grid-cols-[240px_minmax(0,1fr)]">
          ...
          <div className="min-w-0">
            <section className="rise rounded-xl border bg-card p-5 shadow-sm lg:p-6"><div className="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between"><div><p className="font-mono text-[11px] font-bold uppercase tracking-[0.16em] text-primary">Pick one</p><h2 className="mt-1 font-serif text-3xl font-semibold text-destructive">What do you want help with?</h2><p className="mt-1 text-xs text-muted-foreground">Try it free, right now — no account needed. Sign in only if you want to save your results for later.</p></div><Button onClick={() => { setAnalysisError(null); if (isAuthenticated) runAnalysis.mutate({ objective }); else setGuestOpen(true); }} disabled={isAuthenticated && isAnalysing} className="shrink-0">{(isAuthenticated && isAnalysing) ? <Loader2 className="animate-spin" /> : <Sparkles />}{isAuthenticated ? "Check my evidence" : "Check my evidence — no sign-in needed"}</Button></div><div className="mt-5 grid gap-3 lg:grid-cols-3">{(Object.keys(objectives) as Objective[]).map(key => { const item = objectives[key]; const Icon = item.icon; const tint = objectiveIconTint[key]; return <button key={key} onClick={() => chooseObjective(key)} className={`rounded-lg border p-4 text-left shadow-sm transition-all duration-200 hover:-translate-y-0.5 hover:shadow-md ${objective === key ? "border-primary bg-primary/[.035] ring-1 ring-primary/20" : "bg-card hover:border-primary/40"}`}><div className="flex items-center justify-between"><span className={`grid size-8 place-items-center rounded-full ${tint.bg} ${tint.fg}`}><Icon className="size-4" /></span><span className="font-mono text-[11px] font-bold uppercase tracking-[.14em] text-muted-foreground">{item.label}</span></div><p className="mt-4 font-semibold">{item.title}</p><p className="mt-1 text-xs leading-5 text-muted-foreground">{item.description}</p></button>; })}</div></section>
