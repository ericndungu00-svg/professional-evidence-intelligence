...
import { AlertTriangle, ArrowRight, BookOpenText, CheckCircle2, CircleAlert, FolderOpen, Landmark, Moon, Sun, Target, Upload } from "lucide-react";
...
function Qualifications({ active, selectedEvidenceId, setSelectedEvidenceId }: Omit<Props, "objective" | "assessmentByRequirement" | "onAddCurrentRole">) {
  ...
  return <div className="mt-5 rounded-xl border border-rose-200 bg-rose-50/60 p-5 dark:border-rose-900/50 dark:bg-rose-950/20"><div className="flex gap-3"><AlertTriangle className="mt-0.5 size-5 shrink-0 text-rose-800 dark:text-rose-400" /><div><h3 className="font-semibold text-rose-950 dark:text-rose-100">Worth double-checking</h3><div className="mt-3 space-y-3">{active.contradictions.map((item: any) => <div key={item.id ?? item.claim} className="rounded-lg border border-rose-200 bg-background/70 p-3 dark:border-rose-900/50"><p className="text-sm font-medium text-rose-950 dark:text-rose-100">{item.claim}</p><p className="mt-1 text-sm leading-5 text-rose-900 dark:text-rose-200">{item.explanation}</p><SourceLinks active={active} evidenceIds={item.evidenceIds ?? []} selectedEvidenceId={selectedEvidenceId} setSelectedEvidenceId={setSelectedEvidenceId} /></div>)}</div></div></div></div>;
}
...
// Dark-mode status colours -- same hue per outcome as light mode (so the
// meaning doesn't shift), re-tuned to low-opacity dark fills + a lighter
// text tone rather than the light palette's bright pastel fills, so the
// badge reads as a quiet label next to the evidence text rather than
// competing with it for attention.
const alignmentStyle: Record<string, string> = {
  aligned: "bg-emerald-50 text-emerald-800 border-emerald-200 dark:bg-emerald-950/50 dark:text-emerald-300 dark:border-emerald-800/60",
  potentially_broader_responsibility: "bg-violet-50 text-violet-800 border-violet-200 dark:bg-violet-950/50 dark:text-violet-300 dark:border-violet-800/60",
  potentially_narrower_responsibility: "bg-sky-50 text-sky-800 border-sky-200 dark:bg-sky-950/50 dark:text-sky-300 dark:border-sky-800/60",
  insufficient_evidence: "bg-stone-100 text-stone-700 border-stone-200 dark:bg-stone-800/50 dark:text-stone-300 dark:border-stone-700/60",
  unclear_ambiguous: "bg-amber-50 text-amber-800 border-amber-200 dark:bg-amber-950/50 dark:text-amber-300 dark:border-amber-800/60",
};
...
function JobEvaluationReport(props: Props) {
  // Local, presentational only -- an evaluation toggle for this one screen.
  // Nothing else reads or is affected by this state.
  ...
  const section = <section key={props.active?.latestAnalysis?.id ?? "empty"} className="mt-6 rise"><ReportHeader label="Objective C" title="Your role vs. what you actually do" description={`This compares the official responsibilities in ${props.active?.currentRoleDocument?.title ?? "the role description you added"} against what your evidence shows you actually doing day to day. It's not comparing you against the job you're applying for.`} summary={statusSummary} notice={<div className="mt-4 flex gap-3 rounded-lg border border-amber-200 bg-amber-50 p-4 text-sm leading-6 text-amber-900 dark:border-amber-900/50 dark:bg-amber-950/30 dark:text-amber-200"><Landmark className="mt-0.5 size-5 shrink-0" />This is neutral preparation for discussion only. It does not determine a banding/grading decision, entitlement, pay, a job-evaluation outcome, or legal position, under any employer's grading system.</div>} /><div className="mt-4 overflow-hidden rounded-xl border bg-card shadow-sm">{report?.comparisons?.length ? <><p className="flex items-center gap-1 border-b bg-muted/40 px-4 py-2 text-xs text-muted-foreground sm:hidden">Swipe to see all columns<ArrowRight className="size-3" /></p><div className="overflow-x-auto"><table className="report-table min-w-[1120px] w-full text-left text-sm"><thead className="bg-muted/65 text-[11px] uppercase tracking-[.12em] text-muted-foreground"><tr><th className="sticky left-0 z-10 w-[19%] border-r bg-muted/65 px-4 py-3">Your official responsibility</th><th className="w-[22%] px-4 py-3">What you actually do</th><th className="w-[14%] px-4 py-3">Comparison</th><th className="w-[19%] px-4 py-3">Evidence and caveats</th><th className="w-[26%] px-4 py-3">Worth discussing / still needed</th></tr></thead><tbody>{report.comparisons.map((item: any) => <tr key={item.responsibilityId}><td className="sticky left-0 z-10 border-r bg-card px-4 py-4 align-top"><p className="font-semibold">{responsibilityFor(item.responsibilityId)?.criterion ?? "Official responsibility"}</p><p className="mt-1 text-xs text-muted-foreground">{responsibilityFor(item.responsibilityId)?.category}</p></td><td className="px-4 py-4 align-top leading-5 text-muted-foreground">{item.documentedActualActivity}</td><td className="px-4 py-4 align-top"><Badge variant="outline" className={alignmentStyle[item.alignment] ?? alignmentStyle.insufficient_evidence}>{alignmentLabel[item.alignment] ?? "Not enough evidence yet"}</Badge></td><td className="px-4 py-4 align-top"><p className="text-xs leading-5 text-amber-800 dark:text-amber-300">{item.qualification}</p><SourceLinks active={props.active} evidenceIds={item.evidenceIds ?? []} selectedEvidenceId={props.selectedEvidenceId} setSelectedEvidenceId={props.setSelectedEvidenceId} /></td><td className="px-4 py-4 align-top"><p className="text-sm leading-5 text-muted-foreground">{item.discussionPoint}</p><p className="mt-2 rounded bg-secondary/70 px-2.5 py-2 text-xs leading-5 text-secondary-foreground"><strong>Still needed:</strong> {item.strengtheningEvidence}</p></td></tr>)}</tbody></table></div></> : <div className="flex flex-col items-center gap-3 px-6 py-12 text-center">
  ...
  return <>
    <div className="mt-6 flex justify-end">
      <button type="button" onClick={() => setDarkPreview(value => !value)} className="inline-flex items-center gap-1.5 rounded-md border px-3 py-1.5 text-xs font-semibold text-muted-foreground transition-colors hover:bg-muted hover:text-foreground">
        {darkPreview ? <Sun className="size-3.5" /> : <Moon className="size-3.5" />}
        {darkPreview ? "Exit dark mode preview" : "Preview dark mode (exploration)"}
      </button>
    </div>
    {darkPreview ? <div className="dark mt-3 rounded-xl bg-background p-4 text-foreground sm:p-6">{section}</div> : section}
  </>;
}
