...
function SourceLinks({ active, evidenceIds, selectedEvidenceId, setSelectedEvidenceId }: { active: any; evidenceIds: number[]; selectedEvidenceId: number | null; setSelectedEvidenceId: (id: number) => void }) {
  ...
  if (!sources.length) return <p className="mt-3 text-xs text-muted-foreground">No source passage selected for this finding.</p>;
...
function LensCard({ icon: Icon, title, color, items, active, selectedEvidenceId, setSelectedEvidenceId }: any) {
  return <article className="rounded-xl border bg-card p-5 shadow-sm"><div className="flex items-center gap-3"><span className={`grid size-9 place-items-center rounded-md bg-muted ${color}`}><Icon className="size-5" /></span><h3 className="font-serif text-xl font-semibold">{title}</h3></div><div className="mt-5 space-y-5">{items?.length ? items.map((item: any, index: number) => <div key={`${item.title}-${index}`} className="border-l-2 border-border pl-4"><p className="font-semibold text-sm">{item.title}</p><p className="mt-1 text-sm leading-5 text-muted-foreground">{item.statement}</p>{item.qualification && <p className="mt-2 text-xs leading-5 text-amber-800"><strong>Qualification:</strong> {item.qualification}</p>}{item.action && <p className="mt-2 rounded bg-secondary/70 px-2.5 py-2 text-xs leading-5 text-secondary-foreground"><strong>Evidence-building action:</strong> {item.action}</p>}<SourceLinks active={active} evidenceIds={item.evidenceIds ?? []} selectedEvidenceId={selectedEvidenceId} setSelectedEvidenceId={setSelectedEvidenceId} /></div>) : <p className="text-sm text-muted-foreground">No source-linked finding was returned in this section.</p>}</div></article>;
}
...
function PromotionReport(props: Props) {
  ...
  return <section className="mt-6 rise"><ReportHeader label="Objective A" title="Promotion and target role mapping" description="How strongly does the supplied evidence demonstrate each requirement of the selected target role? The detailed requirement-by-requirement view remains available in Evidence Map." /><div className="mt-5 grid gap-5 lg:grid-cols-2"><LensCard icon={CheckCircle2} title="Strongly demonstrated requirements" color="text-emerald-700" items={strongest} {...props} /><LensCard icon={CircleAlert} title="Gaps and evidence-building actions" color="text-amber-700" items={gaps} {...props} /></div><Qualifications {...props} /></section>;
}

function AppraisalReport(props: Props) {
  const report = props.active?.objectiveReports?.B;
  const sections = [
    { icon: CheckCircle2, title: "Documented achievements", color: "text-emerald-700", items: report?.documentedAchievements },
    { icon: Target, title: "Documented impact", color: "text-cyan-700", items: report?.documentedImpact ?? report?.measurableImpact },
    { icon: CircleAlert, title: "Evidence limitations", color: "text-amber-700", items: report?.evidenceLimitations ?? report?.qualifications },
    { icon: BookOpenText, title: "Development themes", color: "text-violet-700", items: report?.developmentThemes },
    { icon: Target, title: "Evidence-grounded objectives", color: "text-primary", items: report?.evidenceGroundedObjectives },
    { icon: BookOpenText, title: "Suggested evidence to retain", color: "text-slate-700", items: report?.suggestedEvidenceToRetain },
  ];
  return <section className="mt-6 rise"><ReportHeader label="Objective B" title="Annual appraisal evidence summary" description="This report keeps documented contribution, documented impact, and evidence limitations distinct. A contribution and a project-level outcome can both be recognised without inferring individual causation or project ownership. It deliberately does not score a target role." /><div className="mt-5 grid gap-5 lg:grid-cols-2">{sections.map(section => <LensCard key={section.title} {...section} {...props} />)}</div><Qualifications {...props} /></section>;
}
...
function JobEvaluationReport(props: Props) {
  ...
  const alignmentLabel: Record<string, string> = { aligned: "Aligned", potentially_broader_responsibility: "Potentially broader responsibility", potentially_narrower_responsibility: "Potentially narrower responsibility", insufficient_evidence: "Insufficient evidence", unclear_ambiguous: "Unclear / ambiguous" };
  const counts = (report?.comparisons ?? []).reduce((all: Record<string, number>, item: any) => ({ ...all, [item.alignment]: (all[item.alignment] ?? 0) + 1 }), {});
  return <section className="mt-6 rise"><ReportHeader label="Objective C" title="Current role responsibility comparison" description={`This report compares the formal/current role responsibilities in ${props.active?.currentRoleDocument?.title ?? "the supplied role description"} with documented actual activity in the evidence library. It does not compare the Band 7 target specification.`} notice={<div className="mt-4 flex gap-3 rounded-lg border border-amber-200 bg-amber-50 p-4 text-sm leading-6 text-amber-900"><Landmark className="mt-0.5 size-5 shrink-0" />This is neutral preparation for discussion only. It does not determine NHS AfC banding, rebanding, entitlement, pay, a job-evaluation outcome, or legal position.</div>} /><div className="mt-5 flex flex-wrap gap-2">{Object.entries(alignmentLabel).map(([key, label]) => <Badge key={key} variant="outline" className={alignmentStyle[key]}>{counts[key] ?? 0} {label}</Badge>)}</div><div className="mt-4 overflow-hidden rounded-xl border bg-card shadow-sm"><div className="overflow-x-auto"><table className="report-table min-w-[1120px] w-full text-left text-sm"><thead className="bg-muted/65 text-[10px] uppercase tracking-[.12em] text-muted-foreground"><tr><th className="w-[19%] px-4 py-3">Formal current-role responsibility</th><th className="w-[22%] px-4 py-3">Documented actual activity</th><th className="w-[14%] px-4 py-3">Neutral comparison</th><th className="w-[19%] px-4 py-3">Source evidence and qualification</th><th className="w-[26%] px-4 py-3">Discussion and evidence still needed</th></tr></thead><tbody>{report?.comparisons?.length ? report.comparisons.map((item: any) => <tr key={item.responsibilityId}><td className="px-4 py-4 align-top"><p className="font-semibold">{responsibilityFor(item.responsibilityId)?.criterion ?? "Formal current-role responsibility"}</p><p className="mt-1 text-xs text-muted-foreground">{responsibilityFor(item.responsibilityId)?.category}</p></td><td className="px-4 py-4 align-top leading-5 text-muted-foreground">{item.documentedActualActivity}</td><td className="px-4 py-4 align-top"><Badge variant="outline" className={alignmentStyle[item.alignment] ?? alignmentStyle.insufficient_evidence}>{alignmentLabel[item.alignment] ?? "Insufficient evidence"}</Badge></td><td className="px-4 py-4 align-top"><p className="text-xs leading-5 text-amber-800">{item.qualification}</p><SourceLinks active={props.active} evidenceIds={item.evidenceIds ?? []} selectedEvidenceId={props.selectedEvidenceId} setSelectedEvidenceId={props.setSelectedEvidenceId} /></td><td className="px-4 py-4 align-top"><p className="text-sm leading-5 text-muted-foreground">{item.discussionPoint}</p><p className="mt-2 rounded bg-secondary/70 px-2.5 py-2 text-xs leading-5 text-secondary-foreground"><strong>Evidence still needed:</strong> {item.strengtheningEvidence}</p></td></tr>) : <tr><td colSpan={5} className="px-4 py-8 text-center text-muted-foreground">Add a current role description and run Objective C to prepare a source-linked comparison.</td></tr>}</tbody></table></div></div><div className="mt-5"><LensCard icon={BookOpenText} title="Neutral questions for discussion" color="text-primary" items={report?.questionsForDiscussion} {...props} /></div><Qualifications {...props} /></section>;
}
